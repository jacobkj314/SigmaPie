...
class MITSL(MTSL):
    ...
    def __init__(
        self, alphabet=None, grammar=None, k=2, data=None, edges=[">", "<"], polar="n"
    ):
    ...
    def learn(self):#updated for MITSL
        ...
        possible = set("".join(c) for c in self.generate_all_ngrams(self.alphabet, self.k*2))

        attested = set()
        for d in self.data:
            d = self.annotate_string(d)
            fourgrams = [d[i:i+4] for i in range(len(d)-3)]
            attested.update(set(fourgrams))
        ...
        b = set("".join(c) for c in self.generate_all_ngrams(self.alphabet, 2))

        for fourgram in unattested:
            p1 = fourgram[:2]; p2 = fourgram[2:]
            c = {p1, p2}
            for s in b:
                if s == p1 or s == p2:
                    continue
        ...
        for tier in self.grammar:
            ...
            if self.check_polarity() == "p":
                ...
                if verbose and not valid:
                    print(string, tier, restrictions)
    ...
    def path(self, string):#updated for MITSL
        ...
        data = [string[i:i+2] for i in range(len(string) - 1)]
        n = len(data)
